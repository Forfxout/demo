const COUNTRIES = [{
    ...
    text: 'Congo, Democratic Republic of the', value: 'CD',
}, {
...
export function getSortedCountries(countryCounts) {
    const countryCountMap = {};
    countryCounts.forEach((item) => { countryCountMap[item.code] = item.count; });
    const sortedCountries = COUNTRIES.slice().sort(
        (a, b) => (countryCountMap[b.value] || 0) - (countryCountMap[a.value] || 0),
    );
